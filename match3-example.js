...
window.onload = function() {
    ...
    var buttons = [ { x: 30, y: 240, width: 150, height: 50, text: "Новая игра"},
                    { x: 30, y: 300, width: 150, height: 50, text: "Показать ход"},
                    { x: 30, y: 360, width: 150, height: 50, text: "Автоигра"}];
    ...
    function drawFrame() {
        ...
        context.font = "24px Verdana";
        context.fillText("Игра три в ряд", 10, 30);
        ...
        for (var i=0; i<buttons.length; i++) {
            if (pos.x >= buttons[i].x && pos.x < buttons[i].x+buttons[i].width &&
                ...
                if (i == 0) {
                    ...
                    // AI Bot
                    aibot = !aibot;
                    buttons[i].text = (aibot?"Выключить":"Включить") + "Авто";
                }
            }
        }
    }
